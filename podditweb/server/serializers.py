...
class LatestAppVersionSerializer(serializers.ModelSerializer):
    ...
    def get_mac_link(self, app_version):
        return self._get_link_root() + '%s/%s/Poddit.app.zip' % (app_version.product.sku, app_version.version)

    def get_windows_link(self, app_version):
        return self._get_link_root() + '%s/%s/Poddit.exe.zip' % (app_version.product.sku, app_version.version)
